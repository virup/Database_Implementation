...
char *Function::GetNameWithoutTable(char* name)
{
	string strTemp;
...
Type Function :: RecursivelyBuild (struct FuncOperator *parseTree, Schema &mySchema) {
	...
	// in this case, we have either a literal value or a variable value, so do a push
		...
		if (parseTree->leftOperand->code == NAME) {
			...
			int myNum = mySchema.Find (GetNameWithoutTable(parseTree->leftOperand->value));
			if (myNum == -1) {
			...
			int myType = mySchema.FindType (GetNameWithoutTable(parseTree->leftOperand->value));
		...
		// in this case, we have a literal value
				...
				*((int *) opList[numOps].litInput) = atoi (GetNameWithoutTable(parseTree->leftOperand->value));
				numOps++;	
				return Int;
				...
				*((double *) opList[numOps].litInput) = atof (GetNameWithoutTable(parseTree->leftOperand->value));
				numOps++;	
				return Double;
		}
