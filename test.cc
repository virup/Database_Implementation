...
void q0 (){
	...
	if(fabs(dummy*3.0-result) > 0.1)
		...
		cout << "Read/Write and last apply is good\n";
...
void q10 (){
	...
	s.Apply(final, relName, 3);  		//failing here
...
void q11 (){
	...
	double result = s.Estimate(final, relName,2);
	cout <<  " result gotten: " << result << endl;
	if(fabs(result-150040)>0.1)
		cout<<"error in estimating Q11...??? \n";
		//answer is given as 150040, multiple people have reported differing answers though
	else
		cout << "Q11 is correct" << endl;
	s.Apply(final, relName,2);
