...
void Record :: PrintToFile (Schema *mySchema, ofstream &o) {
...
void Record :: Print2File (Schema* mySchema, FILE* _out) {
	...
	for (int i = 0; i < n; i++) {
		...
		if (atts[i].myType == Int) {
			int *myInt = (int *) &(bits[pointer]);
			//cout << *myInt;	
			fprintf(_out, "%d", *myInt);

		// then is a double
		} else if (atts[i].myType == Double) {
			double *myDouble = (double *) &(bits[pointer]);
			//cout << *myDouble;	
			fprintf(_out, "%f", *myDouble);

		// then is a character string
		} else if (atts[i].myType == String) {
			char *myString = (char *) &(bits[pointer]);
			//cout << myString;	
			fprintf(_out, "%s", myString);
		} 
